...
class AICDSummary():
  ...
  def plot_summary(self):
    ...
    # Plot raw data
    title='{0} {1} {2} {3}'.format(self.stats.station, self.stats.channel, self.stats.location, self.stats.starttime)
    ...
    #print(self.summary, np.amax(self.summary))
    #print(self.thres, np.amax(self.summary))
    ax2.plot(t,self.summary/np.nanmax(self.summary),c='k')
    ax2.plot(t,self.thres/np.nanmax(self.summary),'--',linewidth=2.0,c='k')
    ...
    plt.tight_layout()
    filename='AIC.'+str(self.stats.station)+'.'+str(self.stats.channel)+'.'+str(self.stats.location)+'.png'
    #plt.show()
